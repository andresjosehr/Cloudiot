...
class SanJavierController extends Controller
{
  ...
  AND mt_time > DATE_SUB((SELECT mt_time FROM log_biofil03 WHERE (mt_name='Biofiltro03--Consumo.Flujo') AND mt_value<>0 ORDER BY mt_time DESC LIMIT 1), INTERVAL 3 HOUR)");
      ...
      public function JavierFlujoDiario(){
                                             ...
                                             MIN(mt_time) AS mt_time
                                              FROM log_biofil03 
                                                WHERE mt_name='Biofiltro03--Consumo.Flujo' 
                                                      AND mt_time > DATE_SUB((SELECT mt_time FROM log_biofil03 WHERE mt_name='Biofiltro03--Consumo.Flujo' ORDER BY mt_time DESC LIMIT 1), INTERVAL 7 DAY)
                                                      AND mt_value<>0
